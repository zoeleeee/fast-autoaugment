...
def target_model(save_path, nb_labels = 2, device='cpu'):
	...
	if save_path and os.path.exists(save_path):
		...
		if 'model' in data or 'state_dict' in data:
			...
			model.load_state_dict({k if 'module.' in k else 'module.'+k: v.to(device) for k, v in data[key].items().to(device)})
		else:
			model.load_state_dict({k: v.to(device) for k, v in data.items()})
		del data
	return model
